...
---+ package JQGaugePlugin

=cut

package Foswiki::Plugins::JQGaugePlugin;
...
sub initPlugin {
    ...
    if ( $Foswiki::cfg{Plugins}{JQueryPlugin}{Enabled} ) {
        ...
        Foswiki::Plugins::JQueryPlugin::registerPlugin( "Gauge",
            'Foswiki::Plugins::JQGaugePlugin::Core' );
            
        Foswiki::Func::registerTagHandler('GAUGE', \&handleGauge);
    }
...
sub handleGauge {
	...
	my $session = shift;
	my $plugin = Foswiki::Plugins::JQueryPlugin::createPlugin('Gauge', $session);
	return $plugin->handleGauge(@_) if $plugin;
	return '';
